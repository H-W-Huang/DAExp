...
def load_dataset():
    ...
    x_target_train = train_images[:20000].astype('float64')
    y_target_train = train_labels[:20000].flatten()
    x_target_test = train_images[:5000].astype('float64')
    y_target_test = train_labels[:5000].flatten()
...
x_evaluate = np.concatenate((x_target_train[:5000],x_target_test[:5000]),axis=0)
f_evaluate = np.concatenate((member_scores[:5000],non_member_scores[:5000]),axis=0)
y_evaluate = np.concatenate((y_train[:5000],y_test[:5000]),axis=0)
l_evaluate = np.zeros([f_evaluate.shape[0]],dtype=np.int)
l_evaluate[0:5000] = 1
# x_evaluate,y_evaluate,l_evaluate = 
...
for i in np.arange(batch_num):
    ...
    X = [ cv2.resize(im, (224, 224)) for im in x_feed]
    x_feed = np.array(X)
    # for j in x_feed:
...
scores_evaluate = model.evaluate(f_evaluate_logits, l_evaluate, verbose=0)  # l_evaluate: l for label
