...
## 1. 限定所有的数据都来自 torchvision 的 svhn
...
class ModelStacker():
    ...
    def load_dataset_rf(self):
        train_loader, test_loader = get_outer_extra_loader(self.BATCH_SIZE)
        return [
    ...
    def load_dataset_nn(self):
        return get_extra_dataloader(self.BATCH_SIZE)


    def load_dataset_lr(self):
        return  get_target_dataloader(self.BATCH_SIZE)
    ...
    def train_lr(self, save_path):
        ...
        train_model(train_loader, test_loader, self.lr, lr=0.001 , epoch=1000)
    ...
    def train_nn(self, save_path, use_FCNN=False):
        ...
        train_model(train_loader, test_loader, self.nn, lr=0.0001 , epoch=15)
    ...
    def _get_numpy_data_from_torch_loader(self,dataloader):
        ...
        y_data =  dataset.labels[min_index:min_index+size]
...
if __name__ == "__main__":
    ...
    # with utils.Tee("modelstacking_nn_svhn_%s.log"%(TODAY)):
        # model_stacker.train_nn("./svhn_nn.pt")

    # with utils.Tee("modelstacking_rf_svhn_%s.log"%(TODAY)):
    #     # model_stacker.load_dataset_rf()
    #     model_stacker.train_rf("./svhn_rf.pkl")

    # with utils.Tee("modelstacking_lr_svhn_%s.log"%(TODAY)):
    #     model_stacker.load_models("","./svhn_rf.pkl","./svhn_nn.pt")
    #     model_stacker.train_lr("./svhn_lr.pt")
    
    target_train_loader, target_test_loader = get_target_dataloader(64)
    model_stacker.load_models("./svhn_lr.pt","./svhn_rf.pkl","./svhn_nn.pt")
    model_stacker.predict(target_train_loader, dump_results=True, save_path = "svhn_victim_member_socres.pkl")
    model_stacker.predict(target_test_loader, dump_results=True, save_path = "svhn_non_victim_member_socres.pkl")
