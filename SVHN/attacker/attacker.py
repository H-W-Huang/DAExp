...
def train_attacker(batch_size, save_model = True):
    ...
    train_x, train_y = make_attacker_dataset(shadow_member_socres[:5000], shadow_non_member_socres)
...
# def run():
#     utils.de_random(0)
#     train_loader, test_loader = get_shadow_dataloader(64)
#     with open("shadow_svhn_15.pt","rb") as f:
#         model = torch.load(f)
...
#     with open("/home/user01/exps/DAMIA/Third_stage/SVHN/pure_model/pure_svhn_15.pt","rb") as f:
#         model = torch.load(f)
#     member_socres = collect_model_outputs(train_loader, model, CUDA=True)
#     non_member_socres = collect_model_outputs(test_loader, model, CUDA=True)
