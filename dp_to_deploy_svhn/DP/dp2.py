...
def load_victim_model_dataset():
    x_data,y_data = load_texas()
    x_data_victim, y_data_victim = slice_dataset(x_data,y_data,0,10000)
    return x_data_victim, y_data_victim
...
def main():
    ...
    repro_str = (
        f"alexnet_svhn_{lr}_{sigma}_"
        f"{max_per_sample_grad_norm}_{BATCH_SIZE}_{epochs}"
    )
    torch.save(run_results, f"run_results_{repro_str}.pt")

    if save_model:
        torch.save(model, "texas_dp.pt")
    tests_model_acc(train_loader, model, CUDA = use_CUDA)
    tests_model_acc(test_loader , model, CUDA = use_CUDA)
    ...
    # attacker = torch.load("lsvhn_attacker_epoch_150.pt",map_location=torch.device(to_device))
    ...
    # tests_model_acc(train_loader, attacker, CUDA=use_CUDA)
