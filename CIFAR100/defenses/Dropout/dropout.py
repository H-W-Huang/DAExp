...
def train_model(train_loader, test_loader, model, epoch=10, lr=0.001, l2=0, print_interval = 5 ):
    ...
    for e in range(1, epoch+1):
        ...
        if e % 5 == 0:
            with open("dropout_cifar100_%d_alex.pt"%(e),"wb") as f:
                torch.save(model, f)
            print("model is saved as "+"dropout_cifar100_%d_alex.pt"%(e))
...
def run():
    ...
    # model = torch.load("dropout_cifar100_100_alex.pt")
    # print(model)
    # tests_model_acc(train_loader,model)
    # tests_model_acc(test_loader,model)
...
if __name__ == "__main__":
    with utils.Tee("dropout_cifar_%s_alex.log"%(TODAY)):
        run()
