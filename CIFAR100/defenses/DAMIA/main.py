...
parser.add_argument('--n_class', type=int, default=110)
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer):
    ...
    for e in range(1, args.n_epoch+1):
        ...
        if e == 1 or e % 5 == 0:
            ...
            print("saving model...")
            with open("cifar_damia_%d.pt"%e,"wb") as f:
                torch.save(model, f)
    with open("cifar_damia_%d_final.pt"%e,"wb") as f:
        torch.save(model, f)
    print("The training takes %ss"%(str(time_cost)))
...
if __name__ == '__main__':
    ...
    with Tee("damia_cifar_%s.log"%(TODAY)):
        ...
        source_loader, source_test_loader = get_extra_dataloader_cifar(args.batchsize,resize_224=False)
            # source_loader, source_test_loader = get_extra_dataloader_cifar(args.batchsize, resize_224=False)
        target_train_loader, target_test_loader = get_target_dataloader_cifar(args.batchsize,resize_224=False)
