...
def train_attacker(batch_size, save_model = True):
    ...
    train_x, train_y = make_attacker_dataset(shadow_member_socres[:4000], shadow_non_member_socres)
    ...
    test_x, test_y = make_attacker_dataset(target_member_socres[:4000], target_non_member_socres)
    ...
    if save_model:
        torch.save(attacker,"cifar100_attacker_epoch_%d.pt"%(epoch))
...
def run():
    ...
    with open("shadow_cifar100_100.pt","rb") as f:
        model = torch.load(f)
