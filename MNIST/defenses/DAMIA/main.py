...
if __name__ == '__main__':
    ...
    source_name = "mnist_temp"
    target_name = "mnist"

    # with Tee("damia_cifar_%s.log"%(TODAY)):
    print('Src: %s, Tar: %s' % (source_name, target_name))

    source_loader, source_test_loader = get_extra_dataloader_mnist(args.batchsize)
    target_train_loader, target_test_loader = get_target_dataloader_mnist(args.batchsize)

    model = models.Transfer_Net(
        args.n_class, transfer_loss="mmd", base_net="basic_nn").to(DEVICE)
